...
function _resolve_object(model::JuMP.Model, index::String)
    return something(JuMP.variable_by_name(model, index), JuMP.constraint_by_name(model, index))
end
...
function _get_result_abstract(node::AbstractNode, object::OptionalObjectIndex, type::AbstractResult)
    ...
    length(node.children) > 1 &&
        (@critical "Cannot get result of multi-children node without additional information" node type)

    return _get_result_child(node, node.children[1], object, type)
end

function get_result(node::AbstractNode, object::OptionalObjectIndex, type::AbstractResult)
...
function _get_result_child(node::AbstractNode, child::AbstractNode, object::OptionalObjectIndex, type::AbstractResult)
    return get_result(child, object, type)
end
function _get_result_self(node::AbstractNode, object::OptionalObjectIndex, type::AbstractResult)
    @critical "Cannot find implementation of `get_result`" node object type
end
