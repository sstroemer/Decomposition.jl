...
function finalize(node::SimpleBendersDecompositionNode)
    ...
    vi = first(main)
    con_with_var = collect(
        c for c in JuMP.all_constraints(model_main; include_variable_in_set_constraints = false) if
        JuMP.normalized_coefficient(c, JuMP.VariableRef(model_main, vi)) != 0
    )
