function lpmd_to_jump(
    ...
    isempty(add_eq) ||
        JuMP.@constraint(jump_model, mat_vec_scalar(x_data_t, A_t[:, idx_eq], 1.0)' .== lpmd.b_lower[add_eq])
    isempty(add_ge) ||
        JuMP.@constraint(jump_model, mat_vec_scalar(x_data_t, A_t[:, idx_ge], 1.0)' .>= lpmd.b_lower[add_ge])
    isempty(add_lt) ||
        JuMP.@constraint(jump_model, mat_vec_scalar(x_data_t, A_t[:, idx_lt], 1.0)' .<= lpmd.b_upper[add_lt])
    ...
    if dualize
        ...
        Dualization.dualize(
            JuMP.backend(jump_model),
            dual_problem;
            variable_parameters = vcat(JuMP.index.(y.data), JuMP.index(θ_star)),
        )
        ...
        dual_jump_model.ext[:dualization_obj_param] = [
            (term.first.a in _params) ? (term.first.a, term.first.b, term.second) :
            (term.first.b, term.first.a, term.second) for term in e_q_obj.terms
        ]

        # Handle MIS-FSZ cut type.
        if has_attribute_type(model, Benders.CutTypeMISFSZ)
            # Get the θ_star parameter in the dualized model.
            dual_jump_model.ext[:dualization_θ_star] =
                JuMP.VariableRef(dual_jump_model, dual_problem.primal_dual_map.primal_parameter[JuMP.index(θ_star)])
            ...
            dual_jump_model.ext[:dualization_π] = [
                elem[2] for elem in dual_jump_model.ext[:dualization_obj_param] if
                elem[2] != dual_jump_model.ext[:dualization_θ_star]
            ]
            ...
            JuMP.@expression(
                dual_jump_model,
                expr_cglp_normalization,
                ω_i * l1_norm_π - ω_0 * dual_jump_model.ext[:dualization_π_0]
            )
